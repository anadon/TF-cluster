...
UpperDiagonalSquareMatrix<u8>* constructCoincidenceMatrix(const double **fullMatrix,
  ...
  for(size_t i = 0; i < n; i++){
    tmpPtr = malloc(sizeof(**intermediateGraph) * fileData.numCols);
    intermediateGraph[i] = (pair<f64, size_t>*) tmpPtr;
    memset(intermediateGraph[i], 0, sizeof(**intermediateGraph) * fileData.numCols);
  }
...
graph<geneData, u8>* constructGraph(UpperDiagonalSquareMatrix<u8> *SCCM, const ED fileData,
  ...
  tr->hintNumVertexes(fileData.numRows);
  tr->hintNumEdges(fileData.numRows * actualNumEdges);
...
double** generateMatrixFromFile(ED fileData, unordered_map<string, char> TFCheck,
  ...
  if(!strcmp(corrTypeLowerCase, "pearson")){
    tr = calculatePearsonCorrelationMatrix((const double**) fileData.exprData, fileData.numGenes,
          fileData.corrVecLeng, fileData.TFCorrData, fileData.TFCorrDataLength);
  }else if(!strcmp(corrTypeLowerCase, "spearman")){
    tr = calculateSpearmanCorrelationMatrix((const double**) fileData.exprData, fileData.numGenes,
          fileData.corrVecLeng, fileData.TFCorrData, fileData.TFCorrDataLength);
  }else if(!strcmp(corrTypeLowerCase, "kendall")){
    tr = calculateKendallsTauCorrelationCorrelationMatrix((const double**) fileData.exprData,
                  fileData.numGenes, fileData.corrVecLeng, fileData.TFCorrData,
                                                    fileData.TFCorrDataLength);
  }else{
    errno = EINVAL;
    tr = NULL;
  }
