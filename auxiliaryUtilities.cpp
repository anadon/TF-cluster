...
void *constructPreSCCMHelper(void *arg){
  ...
  for(size_t i = (numerator * numRows) / denominator;
    ...
    sortDoubleSizeTPairHighToLow<size_t>(intermediateGraph[i], numCols);
  }
...
UpperDiagonalSquareMatrix<u8>* constructCoincidenceMatrix(const CMF &protoGraph,
  ...
  //NOTE: this is kind of 'shifted' forward by 1 in order to omit expressions
  //being perfectly correlated with themselves, because we don't want to include
  //self comparisons.
  for(size_t i = 0; i < n; i++){
    size_t allocSize = sizeof(**intermediateGraph) * (actualNumEdges+1);
  ...
  //Prepare hash tables to check for key collision, and count the
  //collisions to fund the SCCM value in linear time with the hash
  //table size.
  for(size_t i = 0; i < n; i++){
    hashChecks[i].max_load_factor(0.5);
    hashChecks[i].reserve(actualNumEdges);
    for(size_t j = 1; j <= actualNumEdges; j++){
  ...
  //Zero out the x=y diagonal because we don't want to include
  //identities in later analysis.  Just a double checking measure to enforce
  //this constraint, and may be optimized out later upon further testing.  For
  //now, correctness is far more imortant.
  for(size_t i = 0; i < coincidenceMatrix->getSideLength(); i++)
    coincidenceMatrix->setValueAtIndex(i, i, 0);
...
graph<geneData, u8>* constructGraph(UpperDiagonalSquareMatrix<u8> *SCCM,
  ...
  for(size_t i = 0; i < n; i++){
    tmpPtr = realloc(sortedCoincidenceMatrix[i],
                    (actualNumEdges+1) * sizeof(**sortedCoincidenceMatrix));
  ...
  //clen is the number of non-zero elements in a matrix
  for(size_t i = 0; i < n; i++)
    for(size_t j = 0; j < actualNumEdges; j++){
      if(sortedCoincidenceMatrix[i][j].first == 0) continue;
