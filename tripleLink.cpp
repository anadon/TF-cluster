...
graph<geneData, double>* tripleLinkIteration(graph<geneData, double> *geneNetwork,
  ...
  for(size_t i = 0; i < firstVertex->getNumEdges(); i++){
    vertex<geneData, double> *connectedVertex;
      
    connectedVertex = firstVertex->getEdges()[i]->other(firstVertex);
    
    markConnectedVertex(firstVertex->getEdges()[i]->weight, connectedVertex,
                                tripleLink1, tripleLink2, tripleLink3);
  }
  geneNetwork->removeVertex(firstVertex);
  
  for(size_t i = 0; i < secondVertex->getNumEdges(); i++){
    vertex<geneData, double> *connectedVertex;
      
    connectedVertex = secondVertex->getEdges()[i]->other(secondVertex);
    
    markConnectedVertex(secondVertex->getEdges()[i]->weight, connectedVertex,
                                tripleLink1, tripleLink2, tripleLink3);
  ...
  while(!toProcess.empty()){
    ...
    for(size_t i = 0; i < connectedVertex->getNumEdges(); i++){
      vertex<geneData, double> *otherVertex;
      
      otherVertex = connectedVertex->getEdges()[i]->other(connectedVertex);
      ...
      markConnectedVertex(connectedVertex->getEdges()[i]->weight, otherVertex,
                                tripleLink1, tripleLink2, tripleLink3);
